...
// Modified for mod 8 7-17-2026

// Modifications for mod 8 are a new method to modify existing record information and implement serialization as per email from Professor Payne



package com.jankowski.beans;

import java.io.Serializable; // Modified for Mod 8 
import java.sql.*;
...
public class MovieBean implements Serializable { //Modified for Module 8 include that class now implements Serializable 

	//Modified for mod 8 allows for version compatibility if the class fields change later 
    ...
    // **** MODIFICATION FOR MODULE 8 ****
    // Added for Module 8.This method updates an existing record for a movie. the primary key is not changed via this method.its only used for the Where clause.
    // this is to allow the field to be displayed but still be not updatable. 
    public static void updateMovie(MovieBean movie) throws SQLException {
        String sql = "UPDATE max_movies_data SET " +
                "title = ?, genre = ?, release_year = ?, director = ?, rating = ? " +
                "WHERE movie_id = ?";
